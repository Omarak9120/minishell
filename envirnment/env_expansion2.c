	...
	char *key_and_after, t_data *data)
{
	...
	key = ft_strndup(key_and_remaining, ft_strchr
			(key_and_remaining, ' ') - key_and_remaining);
...
char	*concatenvloop(char *input_str, t_data *env_data)
{
	...
	free(input_str);
	updated_input = substitute_key_with_value
		(before_part, key_and_remaining, env_data);
	free(before_part);
